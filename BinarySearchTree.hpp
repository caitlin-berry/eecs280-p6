...
// ---------- DO NOT CHANGE ANYTHING IN THIS FILE ABOVE THIS LINE ----------
  ...
  static int size_impl(const Node *node) {
    ...
    int size = 1 + size_impl(node->left) + size_impl(node->right);
    return size;
  }
  ...
  static int height_impl(const Node *node) {
    ...
    // find the longest path from the node to a leaf
    int longestPath = max(height_impl(node->left), height_impl(node->right));
  ...
  static Node *copy_nodes_impl(Node *node) {
    // base case
    // if there's no node, then the ptr can't pt anywhere
    if (!node) {
      return nullptr;
    }
  ...
  static void destroy_nodes_impl(Node *node) {
    ...
    // get rid of nodes at the left
    destroy_nodes_impl(node->left);
    // get rid of nodes at the right
  ...
  static Node * find_impl(Node *node, const T &query, Compare less) {
    // base case
    // if the tree is empty or element not found...
    if (!node) {
      return nullptr;
    }
    // use the "less" parameter to compare elements
  ...
  static Node* insert_impl(Node* node, const T& item, Compare less) {
    if (node == nullptr) {
        // Create a new node if the current subtree is empty
        return new Node(item, nullptr, nullptr);
    }
    if (less(item, node->datum)) {
        // If item is less than current node, recurse into the left subtree
        node->left = insert_impl(node->left, item, less);
    } else if (less(node->datum, item)) {
        // If item is greater than current node, recurse into the right subtree
        node->right = insert_impl(node->right, item, less);
    }
    // If item is equal to the current node's value, do nothing (no duplicates allowed)
    return node;
  }
  ...
  static Node * min_element_impl(Node *node) {
    // base case
    // if node is empty or doesn't have a left...
    if (!node || !node->left) {
      // ... return current node as min
      return node;
    }
    // tail recursive
  ...
  static Node * max_element_impl(Node *node) {
    // base case
    // if node is empty or doesn't have a right...
    if (!node || !node->right) {
      // ... return current node as max
      return node;
    }
    // tail recursive
  ...
  static bool check_sorting_invariant_impl(const Node* node, Compare less,
                                         const T* min_bound = nullptr,
                                         const T* max_bound = nullptr) {
    if (node == nullptr) {
        return true; 
    }
    // check the current node w the bounds
    if ((min_bound && !less(*min_bound, node->datum)) ||
        (max_bound && !less(node->datum, *max_bound))) {
        return false;
    }
    return check_sorting_invariant_impl(node->left, less, min_bound, &node->datum) &&
           check_sorting_invariant_impl(node->right, less, &node->datum, max_bound);
}
  ...
  static void traverse_inorder_impl(const Node *node, std::ostream &os) {
    ...
    // if it's null...
    if (!node) {
      // ... nothing to traverse & nothing is printed
      return;
    }
    // traverse the left to go through nodes smaller than the current one
    traverse_inorder_impl(node->left, os);
    // process the current node
    // each element is followed by a space
    os << node->datum << " ";
    // traverse the right to go through nodes greater than the current one
  ...
  static Node * min_greater_than_impl(Node *node, const T &val, Compare less) {
    // base case
    // if it's null, there's nothing to process
    if (!node) {
      // no  results can be found
      return nullptr;
    }
    // check if current node is greater than given value
    if (less(val, node->datum)) {
        // recursively search left
        Node* left_result = min_greater_than_impl(node->left, val, less);
        // if valid result is found, return it
        // if not, return current node since its greaeter
        return left_result ? left_result : node;
    }
    // if it's not greater, recursively search right
