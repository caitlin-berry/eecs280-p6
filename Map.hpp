...
private:
  ...
  class PairComp {
  public:
    // compare two elements based on their keys
...
public:
  ...
  Value_type& operator[](const Key_type& k){
    ...
    auto it = tree.find({k, Value_type()});
    // if it's not found...
    if (it == tree.end()) {
      // ... insert a new pair
      tree.insert({k, Value_type()});
      // get the new iterator
      it = tree.find({k, Value_type()});
    }
    // return a reference tothe second value
    return it->second;
  }
  ...
  std::pair<Iterator, bool> insert(const Pair_type &val){
    ...
    if (it != tree.end()) {
      ...
      // false bc insertion didn't happen
      return {it, false};
    }
    // if not...
    // insert it into the tree...
    it = tree.insert(val);
    // ... and return an iterator to the inserted value
    return {it, true};
  }
